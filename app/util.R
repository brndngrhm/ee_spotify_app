...
get_ranked_features <- function(data, feature){
  ...
  df_small <- data %>%
    select(artist_name, album_name, track_name, energy)
  ...
  hchart(df_bar, type = "column", name = hcaes(x = track_name, y = feature_value, alb = album_name, color = artist_color, feature_choice = feature_value)) %>%
    hc_chart(inverted = TRUE) %>%
